...
| 12 | Impresión de patrones de respuestas | Implementado; pendiente de validación | Alta | Los perfiles autorizados pueden abrir directamente la planilla del patrón correspondiente al examen y a cada variante, después de devolverlo y habilitar la revisión de notas. El documento identifica examen, materia, grupo, período y variante, muestra solo número e inciso correcto, organiza 15 preguntas por fila, incluye espacio de firma y sello y se genera temporalmente desde el contenido protegido. La trazabilidad de origen se consulta en una vista de auditoría separada. Respetar las restricciones de acceso y liberación del patrón; registrar la operación en auditoría en la validación integral. Complementa el pendiente 1; visualizar auditoría no equivale a imprimir. |
...
Para la visualización e impresión posterior del patrón se agregó una consulta protegida. El servidor exige que el examen esté en `DEVUELTO`, `PENDIENTE_NOTAS` o `CALIFICADO`, valida el acceso académico al rol —incluido el campus asignado al personal de evaluaciones— y descifra internamente el patrón de cada variante. La interfaz solo muestra las respuestas en modo lectura; no entrega el contenido cifrado ni las claves de protección a perfiles no autorizados.

Este bloque queda en **Pendiente de validación** hasta comprobarlo con una evaluación devuelta aún no calificada, una evaluación ya calificada, un usuario operativo fuera del campus y un perfil sin permisos operativos.
...
4. Cerrar la visualización e impresión de patrones después de la devolución y durante la revisión de notas (1 y 12).
