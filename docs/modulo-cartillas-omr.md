...
## Impresión y auditoría del patrón después de la devolución

En **Lista de Evaluaciones → Documentos**, una evaluación devuelta muestra el botón de impresora **Patrones · firma y sello** cuando está en estado **Devuelto**, **Pendiente de notas** o **Calificado**. El botón genera y abre directamente la planilla imprimible, sin pasar por una previsualización ni mezclar la trazabilidad de auditoría. Las variantes se dibujan consecutivamente en la menor cantidad de páginas posible, con 15 preguntas por fila, y muestran únicamente el número presentado y su inciso correcto. Debajo de cada patrón se imprime la lista de estudiantes que recibió esa variante, con código y nombre completo. Al final del último patrón se agrega un único espacio amplio para la firma y sello del docente.

Junto al botón de impresión se encuentra **Auditoría · previsualizar origen**. Esta acción es independiente y muestra, en modo protegido y de solo lectura, la relación entre el número presentado, la pregunta original del banco, el reactivo y las claves antes y después del barajado. Por ejemplo, permite verificar que la pregunta presentada `1` con respuesta `B` provino de la pregunta `15` del banco con clave original `A`. Esta información no se incluye en la planilla que firma el docente.

El endpoint GET /api/omr/{rolExamenId}/patron-calificado solo responde a ADMINISTRADOR_SISTEMA, RESPONSABLE_EVALUACIONES y PERSONAL_EVALUACIONES con acceso al rol. El backend exige que el examen ya haya sido devuelto y se encuentre en DEVUELTO, PENDIENTE_NOTAS o CALIFICADO, valida el alcance académico —incluido el campus del personal de evaluaciones— y descifra el contenido protegido en el servidor. La respuesta incluye la nómina agrupada por variante para la auditoría autorizada. Nunca devuelve la carga cifrada ni las claves de protección.

El endpoint GET /api/omr/{rolExamenId}/patron-calificado/pdf genera temporalmente un PDF imprimible en oficio vertical (8.5 × 13 pulgadas), con las variantes consecutivas y compactas, 15 preguntas por fila, las respuestas oficiales numeradas y la nómina de cada variante debajo de su patrón. Reutiliza las mismas condiciones de estado, rol y alcance; no guarda una copia adicional del patrón. La impresión queda registrada en la bitácora con el usuario autenticado, la dirección de origen y el estado vigente. La auditoría de origen se consulta separadamente mediante GET /api/omr/{rolExamenId}/patron-calificado.

Desde las nuevas generaciones también se conserva, dentro del contenido protegido, la trazabilidad de cada pregunta: número presentado, número original del banco, identificador del reactivo, clave original del banco y clave resultante después del barajado de opciones. Así, por ejemplo, una respuesta `B` en la pregunta presentada `1` puede auditarse contra la pregunta `15` del banco cuya clave original era `A`. Las generaciones históricas que no incluyan esta relación se identifican como tales y no se reconstruyen con datos actuales del banco.
