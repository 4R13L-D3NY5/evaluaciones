...
export class DashboardComponent implements OnInit {
  ...
  public metricasEstados = computed<EstadoMetrica[]>(() => {
    ...
    const etapas: { estado: DashboardEtapa; backend: RolExamenResponse['estadoFlujo']; color: string; badge: string; icon: string }[] = [
      ...
      { estado: 'Confirmado', backend: 'CONFIRMADO', color: 'bg-teal-600', badge: 'bg-teal-100 text-teal-800 border border-teal-300', icon: 'pi pi-verified' },
  ...
  public getEstadoBadgeClass(etapa: DashboardEtapa): string {
    switch (etapa) {
      ...
      case 'Confirmado': return 'bg-teal-100 text-teal-800 border border-teal-300 font-bold';
