  ...
  header: context {
    ...
    if p > 1 {
      grid(
        columns: (1fr, auto),
        align: (left, right),
        [#text(size: 8pt, fill: luma(90))[VALERIA NICOLE ARISPE BUSTAMANTE · #text(font: "Courier", weight: "bold")[8401928]]],
...
#table(
  ...
  fill: none,
  inset: (x: 5pt, y: 2.2pt),
  [*NOMBRE:* VALERIA NICOLE ARISPE BUSTAMANTE],
  ...
  [
    *CODIGO:* \
    #v(-2pt)
    #align(center)[#text(size: 18pt, weight: "bold")[8401928]]
  ]
)
...
#block(spacing: 3.5pt)[
  *3.* El recurso de alzada ante la Autoridad Regional de Impugnación Tributaria (ARIT) debe interponerse en el plazo perentorio de:
...
#block(spacing: 3.5pt)[
  *4.* El método de determinación de la base imponible sobre base presunta procede cuando:
...
#block(spacing: 3.5pt)[
  *7.* Las multas pagadas por contravenciones tributarias al SIN son consideradas gastos deducibles en la liquidación del IUE.
  #h(10pt) *A)* Verdadero\
  #h(10pt) *B)* Falso\
]

#block(spacing: 3.5pt)[
  *8.* El crédito fiscal generado en compras de combustible (gasolina y diésel) es computable al 100% del valor total de la factura.
  #h(10pt) *A)* Verdadero\
  #h(10pt) *B)* Falso\
]

#block(spacing: 3.5pt)[
  *9.* La depreciación de inmuebles bajo el método de línea recta tiene un coeficiente anual del 2.5% según el D.S. 24051.
  #h(10pt) *A)* Verdadero\
  #h(10pt) *B)* Falso\
]
...
#block(spacing: 3.5pt)[
  *11.* El principio de devengado tributario reconoce ingresos y gastos cuando se generan legalmente, con independencia del cobro o pago.
  #h(10pt) *A)* Verdadero\
  #h(10pt) *B)* Falso\
]

#block(spacing: 3.5pt)[
  *12.* La rectificatoria de una declaración jurada que incrementa el saldo a favor del contribuyente requiere aprobación previa mediante Resolución Administrativa del SIN.
  #h(10pt) *A)* Verdadero\
  #h(10pt) *B)* Falso\
]
...
#block(spacing: 3.5pt)[
  *18.* Son elementos que componen la Deuda Tributaria (DT) según el Artículo 47 del CTB:\
...
#block(spacing: 3.5pt)[
  *23.* Son documentos soporte indispensables en el legajo de auditoría tributaria permanente:\
...
#block(spacing: 3.5pt)[
  *24.* Calcule el interés moratorio generado si transcurrieron 500 días con una tasa de interés del 4% anual sobre el tributo omitido:
...
#block(spacing: 3.5pt)[
  *28.* Determine la Deuda Tributaria consolidada total expresada en Unidades de Fomento de Vivienda (UFV):
...
#block(spacing: 3.5pt)[
  *29.* Al no haber bancarizado las compras de Bs 150.000, ¿cuál es el Crédito Fiscal IVA indebidamente apropiado a reintegrar?
