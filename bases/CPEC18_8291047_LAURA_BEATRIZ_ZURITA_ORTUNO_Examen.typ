  ...
  header: context {
    ...
    if p > 1 {
      grid(
        columns: (1fr, auto),
        align: (left, right),
        [#text(size: 8pt, fill: luma(90))[LAURA BEATRIZ ZURITA ORTUNO · #text(font: "Courier", weight: "bold")[8291047]]],
...
#table(
  ...
  fill: none,
  inset: (x: 5pt, y: 2.2pt),
  [*NOMBRE:* LAURA BEATRIZ ZURITA ORTUNO],
  ...
  [
    *CODIGO:* \
    #v(-2pt)
    #align(center)[#text(size: 18pt, weight: "bold")[8291047]]
  ]
)
...
#block(spacing: 3.5pt)[
  *3.* El método de determinación de la base imponible sobre base presunta procede cuando:
...
#block(spacing: 3.5pt)[
  *4.* En auditoría fiscal, las previsiones para incobrables no admitidas por el D.S. 24051 generan:
...
#block(spacing: 3.5pt)[
  *5.* El recurso de alzada ante la Autoridad Regional de Impugnación Tributaria (ARIT) debe interponerse en el plazo perentorio de:
...
#block(spacing: 3.5pt)[
  *8.* En el Régimen Complementario al IVA (RC-IVA) para dependientes, el Formulario 110 admite facturas de hasta:
...
#block(spacing: 3.5pt)[
  *10.* La depreciación de inmuebles bajo el método de línea recta tiene un coeficiente anual del 2.5% según el D.S. 24051.
  #h(10pt) *A)* Verdadero\
  #h(10pt) *B)* Falso\
]
...
#block(spacing: 3.5pt)[
  *13.* El crédito fiscal generado en compras de combustible (gasolina y diésel) es computable al 100% del valor total de la factura.
  #h(10pt) *A)* Verdadero\
  #h(10pt) *B)* Falso\
]

#block(spacing: 3.5pt)[
  *14.* Las pérdidas tributarias acumuladas en el IUE pueden compensarse de manera indefinida sin límite temporal en empresas no productivas.
  #h(10pt) *A)* Verdadero\
  #h(10pt) *B)* Falso\
]
...
#block(spacing: 3.5pt)[
  *17.* I. Las notas fiscales emitidas por el Sistema Electrónico no requieren impresión física para su validez.\
...
#block(spacing: 3.5pt)[
  *18.* Son documentos soporte indispensables en el legajo de auditoría tributaria permanente:\
...
#block(spacing: 3.5pt)[
  *21.* Son elementos que componen la Deuda Tributaria (DT) según el Artículo 47 del CTB:\
